...
app.post("/ToTable", jsonParser, function(request, response) {
    ...
    var Way = (request.body); // не работает. Надо запарсить и везде позаменять чтобы ок было
...
app.get("/html/place.html", function(request, response) {
...
var num;
app.post("/userandplace", jsonParser, function(request, response) {
    if (!request.body) return response.sendStatus(400);
    console.log(request.body);
    user.place = request.body.place;
...
app.post("/AddCard", jsonParser, function(request, response) {
    if (!request.body) return response.sendStatus(400);
    console.log(request.body);
    user.card = request.body.Card;
...
app.post("/handleE", function(request, response) {
    console.log(request.body)
